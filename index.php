...
<script>
	...
	var programs = ['MA Intercultural Ministry Leadership','MA Intercultural Ministry Studies','MA Intercultural Ministry Education',
		...
		'BA Transformation Entrepreneurship','LEAD'];
	var progCategories =[
		{
			"category":"Undergraduate",
			"programs":["BA Intercultural Ministry Studies and Bible and Theology","AA Intercultural Ministry","Certificate in Bible and Missions","Certificate in Pre-Field Preparation","BA Transformation Entrepreneurship"]
		},
		{
			"category":"Graduate",
	...
	function bootstrapApp() {
		...
		var vm = new Vue({
          ...
          var vmSelf = this;
					...
					if (ownerFilter) { data = filterTerm(ownerFilter,4,5) }
					if (stageFilter){ data = filterTerm(stageFilter,3,4) }
					if (yearFilter){ data = filterTerm(yearFilter,5,6) }
					if (semesterFilter){ data = filterTerm(semesterFilter,6,7) }
